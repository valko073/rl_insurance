...
def generate_agent_model(env=None, lr=.0001, memory_len=100, target_model_update=.09):
    ...
    ag_memory = SequentialMemory(limit=memory_len, window_length=1)
    # ag_policy = BoltzmannQPolicy()
    ag_policy = LinearAnnealedPolicy(EpsGreedyQPolicy(), attr="eps", value_max=.95, value_min=0, value_test=0,
                                     nb_steps=5000)
    ag_dqn = CustomDQNAgent(model=agent_model, nb_actions=env.action_space.n, memory=ag_memory, nb_steps_warmup=100,
                      target_model_update=target_model_update, policy=ag_policy)
    ag_dqn.compile(Adam(lr=lr), metrics=['mae'])
...
def generate_insurance_model(env=None, lr=.0001, memory_len=100, target_model_update=.09):
